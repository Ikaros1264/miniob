...
RC LogicalPlanGenerator::create_plan(SelectStmt *select_stmt, unique_ptr<LogicalOperator> &logical_operator)
{
  ...
  if(order_by.size()){
    order_by_oper = make_unique<OrderByLogicalOperator>(std::move(order_by), std::move(select_stmt->is_asc()));
    if (*last_oper) {
...
RC LogicalPlanGenerator::create_plan(InsertStmt *insert_stmt, unique_ptr<LogicalOperator> &logical_operator)
{
  ...
  std::vector<std::vector<Value>> values_set = insert_stmt->values_set();

  InsertLogicalOperator *insert_operator = new InsertLogicalOperator(table, std::move(values_set));
...
RC LogicalPlanGenerator::create_plan(UpdateStmt *update_stmt, std::unique_ptr<LogicalOperator> &logical_operator)
{
  ...
  logical_operator = std::make_unique<UpdateLogicalOperator>(
      update_stmt->table(), update_stmt->field_meta(), update_stmt->value());
