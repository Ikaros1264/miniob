...
RC HeapTableEngine::create_index(Trx *trx, bool unique, const FieldMeta *field_meta, const char *index_name)
{
  ...
  rc = index->create(table_, index_file.c_str(), unique, new_index_meta, *field_meta);
  if (rc != RC::SUCCESS) {
    delete index;
