...
RC BplusTreeIndex::create(Table *table, const char *file_name, const bool unique, const IndexMeta &index_meta, const FieldMeta &field_meta)
{
  if (inited_) {
  ...
  RC rc = index_handler_.create(table->db()->log_handler(), bpm, file_name, unique, field_meta.type(), field_meta.len());
  if (RC::SUCCESS != rc) {
